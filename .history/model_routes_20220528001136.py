...
def book_appointment(pid:int,did:int,db:Session=Depends(get_db)):
    ...
    status=session.query(Appointments.status).filter(Appointments.p_id==pid).filter(Appointments.d_id==).all()
    if patient and doctor and :
        appointment=Appointments(p_id=pid,d_id=did,status=1)
