...
async def register_doctor(name:str,email:EmailStr,phone:str,qualification:str,description:str,expertise,db: session = Depends(get_db)) -> JSONResponse:
     email=email.upper()
...
async def register_patient(name:str,email:EmailStr,phone:str,blood_type:str,description:str,db: session = Depends(get_db)) -> JSONResponse:
     email=email.upper()
     db_cred=session.query(Credentials).filter(Credentials.email==email).first()
