...
@auth_router.post("/book appointment/{p_id}")
def book_appointment(pid:int,did:int,date:str,time:str,db:Session=Depends(get_db)):
    patient=session.query(Patient).filter(Patient.p_id==pid).first()
    doctor=session.query(Doctor).filter(Doctor.d_id==did).first()
    if patient and doctor:
        appointment=Appointment(p_id=pid,d_id=did,date=date,time=time)
