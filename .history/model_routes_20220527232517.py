...
async def register_patient(name:str,email:EmailStr,phone:str,blood_type:str,dob:date,gender:str,db: session = Depends(get_db)) -> JSONResponse:
     email=email.upper()
     db_cred=session.query(Credentials).filter(Credentials.email==email).first()
...
async def doctor_login(email:str,password:str,Authorize:AuthJWT=Depends()):
    email=email.upper()
    db_user=session.query(Credentials).filter(email==Credentials.usn).first()
